...
export const onAssign = async (email, fullname, course) => {
    try {
        let info = await transporter.sendMail({
            from: "Developer of B.E.S.T. Project", // sender address
            to: email,
            subject: "การมอบหมายงาน 👻 👻", // Subject line
            html:
                `
                <html lang="en">
                <head>
                ...
                <body>
                    <div class="container">
                        <div class="card">
                            <div class="card-title">สวัสดี</div>
                            <div class="content">
                                คุณ <b> ${fullname} </b> รายวิชา <b> ${course} </b> ที่ท่านได้เข้าร่วมการเรียน
                                 มีการมอบหมายงานให้ท่านใหม่
                            </div>
                            <div class="text-footer">อีเมลนี้ถูกส่งด้วยระบบอัตโนมัติ กรุณาอย่าตอบกลับอีเมลนี้</div>
                        </div>
                    </div>
...
export const onLecture = async (email, fullname, course) => {
    try {
        let info = await transporter.sendMail({
            from: "Developer of B.E.S.T. Project", // sender address
            to: email,
            subject: "เนื้อหาการเรียนใหม่ 👻 👻", // Subject line
            html:
                `
                <html lang="en">
                <head>
