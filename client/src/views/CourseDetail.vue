  ...
  methods: {
    ...
    async createChat(receiverId) {
      try {
        ...
        await axios.post(import.meta.env.VITE_ENDPOINT + '/api/chat/create', {
    ...
    async createAssignment() {
      ...
      try {
        await axios.post(import.meta.env.VITE_ENDPOINT + '/api/assignment/createAssignment', data)
        this.visible = false
    ...
    async createLecture() {
      ...
      try {
        await axios.post(import.meta.env.VITE_ENDPOINT + '/api/lecture/createLecture', data)
    ...
    async deleteAssignment(id) {
      try {
        await axios.delete(import.meta.env.VITE_ENDPOINT + `/api/assignment/deleteAssignment/${id}`)
    ...
    async deleteLecture(id) {
      try {
        await axios.delete(import.meta.env.VITE_ENDPOINT + `/api/lecture/deleteLecture/${id}`)
    ...
    async createPost() {
      try {
        await axios.post(import.meta.env.VITE_ENDPOINT + '/api/post/createPost', {
          courseId: this.id,
    ...
    async deletePost(id) {
      try {
        await axios.delete(import.meta.env.VITE_ENDPOINT + `/api/post/deletePost/${id}`)
    ...
    async createComment() {
      try {
        await axios.post(import.meta.env.VITE_ENDPOINT + '/api/comment/createComment', {
          postId: this.postId,
          authorId: this.user.id,
          content: this.centent
        })
    ...
    async deleteComment(id) {
      console.log(id)
      try {
        await axios.delete(import.meta.env.VITE_ENDPOINT + `/api/comment/deleteComment/${id}`)
    ...
    async exitCourse(id) {
      console.log(id)
      try {
        await axios.post(import.meta.env.VITE_ENDPOINT + `/api/enrollment/exitCourse`, {
          courseId: this.id,
          userId: this.user.id
        })
        this.$toast.add({ severity: 'warn', summary: 'สำเร็จ', detail: 'ลบคอมเมนต์สำเร็จ!', life: 3000 });
    ...
    async getCourse() {
      try {
        const res = await axios.get(import.meta.env.VITE_ENDPOINT + '/api/course/getCourse/:id', {
          params: {
            id: this.id
          }
        })
