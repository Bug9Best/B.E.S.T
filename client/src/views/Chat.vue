...
export default {
  ...
  mounted() {
    this.getRoomChat()
    io('http://ec2-3-89-104-54.compute-1.amazonaws.com:4000')
      .on('sendMessage', () => {
  ...
  methods: {
    ...
    async getRoomChat() {
      try {
        const res = await axios.get(`http://ec2-3-89-104-54.compute-1.amazonaws.com:8080/api/user/chat/${this.userId}`)
        this.rooms = res.data
      } catch (error) {
        console.log(error)
      }
    ...
    async getMessage(chatId) {
      try {
        const res = await axios.get(`http://ec2-3-89-104-54.compute-1.amazonaws.com:8080/api/chat/message/${chatId}`)
    ...
    async sendMessage() {
      try {
        await axios.post(`http://ec2-3-89-104-54.compute-1.amazonaws.com:8080/api/chat/message`, {
